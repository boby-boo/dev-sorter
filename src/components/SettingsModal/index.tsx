...
import { resetGame, setBalls, setIsSettingsOpen } from "../../features/game/gameConfigSlice";
...
const SettingsModal = () => {
    ...
    return createPortal(
        <div className="settings-modal" onClick={() => dispatch(setIsSettingsOpen(false))}>
            <div className="settings-modal__content" onClick={(e) => e.stopPropagation()}>
                ...
                <ul className="settings-modal__list">
                    {ballsItems.map((item) => {
                        ...
                        const currentClass = isActive ?  'selected__button' : `settings-modal-button_${item.id}`

                        return (
                            <li key={item.label} className="settings-modal__item">
                                <button 
                                className={`settings-modal__button ${currentClass}`}
                                onClick={() => handleClick(item.id)}>
                                    <span className="settings-modal__button-text">{item.label}</span>
                                </button>
                            </li>
                        )
                    })}
                ...
                <div className="settings-modal__button-container">
                    <Button text="Apply" disabled={activeButtonsId.length < 3} onClick={handleApply} />
                </div>
            </div>
        </div>,
